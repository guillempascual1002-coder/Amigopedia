...
const AVAILABLE_CARD_SET = new Set(allAvailableCards);
...
const CARD_DESCRIPTIONS = {
    ...
    "especiales-02.png": "Gonzalo, enemigo público número 1",
    "especiales-03.png": "La abuela de Julia, \"las abuelas de mis amigas, son mis amigas\"",
...
function cardFileExists(filename) {
    return AVAILABLE_CARD_SET.has(filename);
}
...
function createDepthLayers(filename) {
    ...
    const underClip = document.createElement('div');
    underClip.className = 'depth-under-clip';

    const underLayers = document.createElement('div');
    underLayers.className = 'depth-under';
    ...
    middleImg.className = 'depth-middle';
    middleImg.src = 'images/' + filename;
    middleImg.loading = 'lazy';
    middleImg.decoding = 'async';
    middleImg.alt = filename;
    middleImg.dataset.filename = filename;

    const topImg = document.createElement('img');
    topImg.className = 'depth-top';
    topImg.src = 'images/up.png';
    topImg.loading = 'lazy';
    topImg.decoding = 'async';
    topImg.alt = 'up';
    topImg.setAttribute('aria-hidden', 'true');

    underLayers.appendChild(baseImg);
    underLayers.appendChild(middleImg);
    underClip.appendChild(underLayers);
    depth.appendChild(underClip);
    depth.appendChild(topImg);

    return { depth, displayImg: middleImg };
}
...
function createCardElement(filename, wrapperClass, { wrapNonHolo = false, allowHolo = false } = {}) {
    ...
    const getCardContent = () => {
        ...
        const { depth, displayImg } = createDepthLayers(filename);
        return { node: depth, displayImg };
    };
...
function normalizeCardLayersForAlbum(cardNode) {
    if (!cardNode || typeof cardNode.querySelector !== 'function') return;
    const depthBase = cardNode.querySelector('.depth-base');
    const depthMiddle = cardNode.querySelector('.depth-middle');
    const depthTop = cardNode.querySelector('.depth-top');
    const depthUnderClip = cardNode.querySelector('.depth-under-clip');
    const holoOverlay = cardNode.querySelector('.holo-overlay');

    if (depthBase) depthBase.style.transform = 'translateZ(0px)';
    if (depthMiddle) depthMiddle.style.transform = 'translateZ(16px)';
    if (depthTop) depthTop.style.transform = 'translateZ(20px)';
    if (depthUnderClip) depthUnderClip.style.transform = '';
    if (holoOverlay) {
        holoOverlay.style.transform = '';
        holoOverlay.style.opacity = '';
        holoOverlay.style.maskImage = '';
        holoOverlay.style.webkitMaskImage = '';
    }
}

// add a single card file to its exact slot if not already present
function addCard(filename) {
    if (!cardFileExists(filename)) return;
    const group = parseGroup(filename);
    const num = parseSlotNumber(filename);
...
function attachMobileInspectPointer(wrapper, reflection, holoOverlay) {
    ...
    const depthMiddle = wrapper.querySelector('.depth-middle');
    const depthTop = wrapper.querySelector('.depth-top');
    const depthUnderClip = wrapper.querySelector('.depth-under-clip');

    const applyLayerParallax = (shiftX, shiftY) => {
        if (depthBase) depthBase.style.transform = `translate3d(${shiftX * 0.3}px, ${shiftY * 0.3}px, 0)`;
        if (depthMiddle) depthMiddle.style.transform = `translate3d(${shiftX * 1}px, ${shiftY * 1}px, 0)`;
        if (depthTop) depthTop.style.transform = `translate3d(${shiftX * 0}px, ${shiftY * 0}px, 0)`;
        if (depthUnderClip) depthUnderClip.style.transform = `translate3d(${shiftX * 0}px, ${shiftY * 0}px, 0)`;
        if (holoOverlay) holoOverlay.style.transform = `translate3d(${shiftX * 0}px, ${shiftY * 0}px, 0)`;
    };

    const resetLayerParallax = () => {
        if (depthBase) depthBase.style.transform = 'translate3d(0, 0, 0)';
        if (depthMiddle) depthMiddle.style.transform = 'translate3d(0, 0, 0)';
        if (depthTop) depthTop.style.transform = 'translate3d(0, 0, 0)';
        if (depthUnderClip) depthUnderClip.style.transform = 'translate3d(0, 0, 0)';
        if (holoOverlay) holoOverlay.style.transform = 'translate3d(0, 0, 0)';
    };
...
function tryPlaceInBattleZone(img, x, y) {
    ...
    const battleToggle = document.getElementById('battle-drawer-toggle');
    const draggedFilename = (getCardInfoFromImg(img).filename || '').toLowerCase();
    const leftFilename = battleController.leftImg ? (getCardInfoFromImg(battleController.leftImg).filename || '').toLowerCase() : '';
    const rightFilename = battleController.rightImg ? (getCardInfoFromImg(battleController.rightImg).filename || '').toLowerCase() : '';
    ...
    if (pointInRect(x, y, leftRect) && !battleController.leftImg) {
        if (draggedFilename && rightFilename && draggedFilename === rightFilename) return false;
        const clone = img.cloneNode(true);
    ...
    if (pointInRect(x, y, rightRect) && !battleController.rightImg) {
        if (draggedFilename && leftFilename && draggedFilename === leftFilename) return false;
        const clone = img.cloneNode(true);
        clone.removeAttribute('style');
        clone.classList.remove('dragging', 'returning');
        clone.style.pointerEvents = 'none';
...
function closeDisplayZone() {
    ...
    if (displayState.card) {
        ...
        normalizeCardLayersForAlbum(card);
...
function setupDisplayCardEffects(wrapper, reflection) {
    ...
    const depthBaseLayer = wrapper.querySelector('.depth-base');
    const depthMiddleLayer = wrapper.querySelector('.depth-middle');
    const depthTopLayer = wrapper.querySelector('.depth-top');
    const depthUnderClipLayer = wrapper.querySelector('.depth-under-clip');
    ...
    const handleMove = (e) => {
        ...
        if (depthBaseLayer || depthMiddleLayer || depthTopLayer) {
            ...
            if (depthBaseLayer) {
                depthBaseLayer.style.transform = `translateZ(0px) rotateX(${layerRotateX * 0.25}deg) rotateY(${layerRotateY * 0.25}deg) translate3d(${parallaxX * 0.5}px, ${parallaxY * 0.5}px, 0)`;
            }
            if (depthMiddleLayer) {
                depthMiddleLayer.style.transform = `translateZ(16px) rotateX(${layerRotateX * 0.7}deg) rotateY(${layerRotateY * 0.7}deg) translate3d(${parallaxX * 1.6}px, ${parallaxY * 1.6}px, 0)`;
            }
            if (depthTopLayer) {
                depthTopLayer.style.transform = `translateZ(30px) rotateX(${layerRotateX}deg) rotateY(${layerRotateY}deg) translate3d(${parallaxX * 0}px, ${parallaxY * 0}px, 0)`;
            }
            if (depthUnderClipLayer) {
                depthUnderClipLayer.style.transform = `translateZ(30px) rotateX(${layerRotateX}deg) rotateY(${layerRotateY}deg) translate3d(${parallaxX * 0}px, ${parallaxY * 0}px, 0)`;
            }
            if (holoOverlay) {
                holoOverlay.style.transform = `translateZ(30px) rotateX(${layerRotateX}deg) rotateY(${layerRotateY}deg) translate3d(${parallaxX * 0}px, ${parallaxY * 0}px, 0)`;
            }
        }
    ...
    const handleLeave = () => {
        ...
        if (holoOverlay) {
            ...
            holoOverlay.style.maskImage = 'none';
            holoOverlay.style.webkitMaskImage = 'none';
            holoOverlay.style.transform = 'translateZ(30px)';
        }
        if (depthBaseLayer) {
            depthBaseLayer.style.transform = 'translateZ(0px)';
        }
        if (depthMiddleLayer) {
            depthMiddleLayer.style.transform = 'translateZ(16px)';
        }
        if (depthTopLayer) {
            depthTopLayer.style.transform = 'translateZ(30px)';
        }
        if (depthUnderClipLayer) {
            depthUnderClipLayer.style.transform = 'translateZ(30px)';
        }
    };
...
function loadCollection() {
    try {
        ...
        Object.keys(collection).forEach(group => {
            ...
            Object.keys(data[group]).forEach(numKey => {
                const num = parseInt(numKey, 10);
                ...
                const incoming = data[group][numKey] || {};
                const quantity = Math.max(0, Number(incoming.quantity) || 0);
                const wins = Math.max(0, Number(incoming.wins) || 0);

                if (quantity > 0 || wins > 0) {
                    sanitizedGroup[num] = { quantity, wins };
                }
            });

            collection[group] = sanitizedGroup;
        });
